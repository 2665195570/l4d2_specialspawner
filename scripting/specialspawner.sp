...
public Plugin myinfo = {
	...
	author = "Tordecybombo, breezy & Neko Channel (merged)",
...
public void OnPluginStart() {
	...
	g_cSpawnLimits[SI_SMOKER] = 	CreateConVar("ss_smoker_limit",			"8",						"同时存在的最大smoker数量", _, true, 0.0, true, 32.0);
	g_cSpawnLimits[SI_BOOMER] = 	CreateConVar("ss_boomer_limit",			"8",						"同时存在的最大boomer数量", _, true, 0.0, true, 32.0);
	g_cSpawnLimits[SI_HUNTER] = 	CreateConVar("ss_hunter_limit",			"8",						"同时存在的最大hunter数量", _, true, 0.0, true, 32.0);
	g_cSpawnLimits[SI_SPITTER] = 	CreateConVar("ss_spitter_limit",		"8",						"同时存在的最大spitter数量", _, true, 0.0, true, 32.0);
	g_cSpawnLimits[SI_JOCKEY] = 	CreateConVar("ss_jockey_limit",			"8",						"同时存在的最大jockey数量", _, true, 0.0, true, 32.0);
	g_cSpawnLimits[SI_CHARGER] = 	CreateConVar("ss_charger_limit",		"8",						"同时存在的最大charger数量", _, true, 0.0, true, 32.0);
	...
	g_cSpawnTimeMin =				CreateConVar("ss_time_min",				"25.0",						"特感的最小产生时间", _, true, 0.1);
	g_cSpawnTimeMax =				CreateConVar("ss_time_max",				"45.0",						"特感的最大产生时间", _, true, 1.0);
	...
	g_cExtraLimit =					CreateConVar("ss_extra_limit",			"2",						"生还者团队每增加一人可增加多少个特感", _, true, 0.0, true, 32.0);
	...
	g_cSpawnRangeMin =				CreateConVar("ss_spawnrange_min",		"1500.0",					"特感最小生成距离", _, true, 0.0);
	g_cSpawnRangeMax =				CreateConVar("ss_spawnrange_max",		"3500.0",					"特感最大生成距离", _, true, 0.0);

	g_cFirstSpawnTime = 			CreateConVar("ss_first_time",			"25.0",						"玩家离开安全区域后第一波特感的刷新时间", _, true, 0.0);

	// ========== 新增 ConVar ==========
	g_cUseDirChance =				CreateConVar("ss_use_dirchance",		"1",						"启用每个特感的独立方位概率 [0=关闭(使用原方向逻辑) | 1=开启]", _, true, 0.0, true, 1.0);
	g_cSpawnDirChance[SI_SMOKER] =	CreateConVar("ss_smoker_dirchance",		"100",						"Smoker刷新方位概率(数值越大越靠前) [1-100]", _, true, 1.0, true, 100.0);
	g_cSpawnDirChance[SI_BOOMER] =	CreateConVar("ss_boomer_dirchance",		"100",						"Boomer刷新方位概率(数值越大越靠前) [1-100]", _, true, 1.0, true, 100.0);
	g_cSpawnDirChance[SI_HUNTER] =	CreateConVar("ss_hunter_dirchance",		"100",						"Hunter刷新方位概率(数值越大越靠前) [1-100]", _, true, 1.0, true, 100.0);
	g_cSpawnDirChance[SI_SPITTER] =	CreateConVar("ss_spitter_dirchance",	"100",						"Spitter刷新方位概率(数值越大越靠前) [1-100]", _, true, 1.0, true, 100.0);
	g_cSpawnDirChance[SI_JOCKEY] =	CreateConVar("ss_jockey_dirchance",		"100",						"Jockey刷新方位概率(数值越大越靠前) [1-100]", _, true, 1.0, true, 100.0);
	g_cSpawnDirChance[SI_CHARGER] =	CreateConVar("ss_charger_dirchance",	"100",						"Charger刷新方位概率(数值越大越靠前) [1-100]", _, true, 1.0, true, 100.0);
	g_cTankBlockSpawn =				CreateConVar("ss_tank_block_spawn",		"1",						"坦克存活时是否禁止刷特 [0=允许 | 1=禁止]", _, true, 0.0, true, 1.0);
	// ==================================
	...
	for (int i; i < SI_MAX_SIZE; i++) {
		...
		// 新增：方位概率钩子
		g_cSpawnDirChance[i].AddChangeHook(CvarChanged_General);
	}
	...
	// 新增：开关钩子
	g_cUseDirChance.AddChangeHook(CvarChanged_General);
	g_cTankBlockSpawn.AddChangeHook(CvarChanged_General);

	AutoExecConfig(true, "specialspawner");
...
void GetCvars_General() {
	...
	for (int i; i < SI_MAX_SIZE; i++) {
		...
		// 新增：读取方位概率
		g_iSpawnDirChance[i] = g_cSpawnDirChance[i].IntValue;
	}
	...
	// 新增
	g_bUseDirChance =	g_cUseDirChance.BoolValue;
	g_bTankBlockSpawn =	g_cTankBlockSpawn.BoolValue;
}
...
void ExecuteSpawnQueue(int totalSI, bool retry) {
	// ===== 新增：坦克禁止刷特检查 =====
	if (g_bTankBlockSpawn && FindTank(-1)) {
		#if DEBUG
		PrintToServer("[SS] Tank alive and block spawn is ON, skip spawn.");
		#endif
		return;
	}
	// =================================
